...
```json agent-policy
{
  ...
  "resolution": {
    "partnerAnswer": "Откройте программу «Тест драйвера ККТ» в меню «Пуск». Поставьте галочку «Включено» справа в окне программы. Перейдите в раздел «Отчеты», выберите «Отчёт о закрытии смены (с гашением)» и нажмите «Сформировать». Сохраните вышедший Z-отчёт, закройте драйвер и продолжайте работу в кассовом приложении. При ошибке «Нет доступа к ККМ» выберите «Вид» → «Обновить».",
  ...
  "readiness": {
    ...
    "evidence": ["общая статья БЗ", "чаты операторов за август 2026 года", "подтверждение владельца проекта"],
    "blockers": ["пройти регрессионный тест после синхронизации статьи и кода"]
  }
}
```
