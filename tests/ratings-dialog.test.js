...
async function main() {
  ...
  // Тип рейтинга спрашивается не более двух раз; наугад компонент и подзадача не ставятся.
  c = bot("rko", "no_answer");
  await c.turn("Тамбов-1, хотим оспорить рейтинг", { unit: "Тамбов-1" });
  await c.turn("Не знаю, какой именно");
  r = await c.turn("Уточнить не получается");
  t.check("неопределённый рейтинг после двух вопросов передаётся оператору",
    c.turns.length === 3 && r.stage === "escalated" && r.internal.length === 1, r);
