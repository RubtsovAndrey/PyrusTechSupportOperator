...
- `ID_Tools.searchKnowledge` — Ищет в базе знаний тематику, подходящую под описание проблемы. Если ничего не подходит, возвращает found=false без кандидатов и никогда не угадывает. Когда тематика уже известна, передавай topicKey. Для choose-branch с activeQuestionId интерпретируй текущую реплику в одно из answerValues и повторно вызови tool с activeQuestionId, answerValue и дословным evidenceText.
